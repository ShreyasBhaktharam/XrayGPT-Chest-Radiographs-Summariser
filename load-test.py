...
def send_request(payload, request_num, total_requests):
    ...
    print(f"\nRequest {request_num}/{total_requests} STARTED at {time.strftime('%H:%M:%S')}")
    retry_count = 0
    
    while retry_count < MAX_RETRIES:
        try:
            print(f"Sending request {request_num}/{total_requests}...")
...
def run_single_batch_test():
    """Run a batch of requests using multiple workers."""
    print(f"\n=== Starting Test with {NUM_WORKERS} Workers and {TOTAL_REQUESTS} Total Requests ===")
    
    # Create unique payloads for each request
    payloads = [create_test_payload() for _ in range(TOTAL_REQUESTS)]
    
    # Process requests concurrently using worker threads
    with concurrent.futures.ThreadPoolExecutor(max_workers=NUM_WORKERS) as executor:
        futures = []
        for i, payload in enumerate(payloads):
            request_num = i + 1
            futures.append(executor.submit(send_request, payload, request_num, TOTAL_REQUESTS))
...
def main():
    ...
    # Run the test with configured workers and requests
    inference_times, failed_requests = run_single_batch_test()
    ...
    if len(inference_times) > 0:
        ...
        print("\nPerformance Metrics:")
        print(f"Total requests completed: {len(inference_times)}/{TOTAL_REQUESTS}")
        print(f"Failed requests: {failed_requests}")
        print(f"Success rate: {(len(inference_times)/TOTAL_REQUESTS)*100:.2f}%")
