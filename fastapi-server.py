...
async def generate(request: GenerateRequest):
    ...
    try:
        ...
        pil_image = decode_image(request.image)
        ...
        img_list = []
        chat.upload_img(pil_image, conversation, img_list)
        
        # Add the user's message to the conversation
        chat.ask(request.message, conversation)
...
async def generate(request: GenerateRequest):
    ...
    try:
        ...
        logger.info("Decoding image")
        # pil_image = decode_image(request.image)
        # Consider running decode in threadpool if images are very large or decoding is complex
        ...
        def process_and_generate_sync(pil_image, conversation, img_list, message, params):
            ...
            return llm_response[0] if isinstance(llm_response, (list, tuple)) else llm_response

        # Run the blocking operations in the thread pool
        response_text = await run_in_threadpool(
            process_and_generate_sync,
            pil_image=pil_image,
            conversation=conversation, # Pass the conversation object
            img_list=img_list,         # Pass the img_list
        ...
        GENERATION_ERRORS.inc()
        logger.error(f"Error during processing: {str(e)}", exc_info=True) # Log traceback
