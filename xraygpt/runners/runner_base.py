...
class RunnerBase:
    ...
    def train(self):
        ...
        mlflow.log_params({
            "batch_size_train": self.config.run_cfg.batch_size_train,
            "batch_size_eval": self.config.run_cfg.batch_size_eval,
            "init_lr": self.config.run_cfg.init_lr,
            "min_lr": self.config.run_cfg.min_lr,
            "max_epoch": self.config.run_cfg.max_epoch,
            "weight_decay": self.config.run_cfg.weight_decay,
            "model_type": self.config.model_cfg.get("type", "unknown"),
        })
        
        # Log system info
        mlflow.log_param("num_gpus", torch.cuda.device_count() if torch.cuda.is_available() else 0)
        mlflow.log_param("distributed_training", self.config.run_cfg.distributed)
                ...
                if is_main_process():
                    for k, v in train_stats.items():
                        if isinstance(v, (int, float)):
                            mlflow.log_metric(f"train_{k}", v, step=cur_epoch)
                    ...
                    if val_log is not None:
                        if is_main_process():
                            ...
                            for k, v in val_log.items():
                                if isinstance(v, (int, float)):
                                    mlflow.log_metric(f"val_{k}", v, step=cur_epoch)
        ...
        test_logs = self.evaluate(ckpt=self.resume_ckpt_path, cur_epoch=test_epoch, skip_reload=self.evaluate_only)

        # Log test metrics to MLflow
        if test_logs and is_main_process():
            for split_name, split_logs in test_logs.items():
                for k, v in split_logs.items():
                    if isinstance(v, (int, float)):
                        mlflow.log_metric(f"test_{split_name}_{k}", v)
